...
class Ultrasonic {
  ...
  private:
    ...
    const int timeDistOffset = 482;       //Subtract this constant from the time delay to get the distance. Value determined experimentally.
    int trigPin;                          //Sensor trigger pin
    int echoPin;                          //Sensor echo pin
    unsigned long triggerTime;            //Time measured by echoInterruptRising
    volatile unsigned long timeDiff;      //Delay of echo signal in microseconds
